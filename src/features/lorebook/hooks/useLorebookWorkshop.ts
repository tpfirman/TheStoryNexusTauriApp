    ...
    const generate = useCallback(async (agentPreset: AgentPreset, history: PromptMessage[]) => {
        ...
        try {
            ...
            await aiService.processStreamedResponse(
                ...
                () => {
                    console.debug(`[lorebook] onComplete: fullText(${fullText.length}) first80=${JSON.stringify(fullText.slice(0, 80))}`);
                    const { proseText } = splitThinkingContent(fullText);
                    console.debug(`[lorebook] proseText(${proseText.length}) first80=${JSON.stringify(proseText.slice(0, 80))}`);
                    const { entries } = parseLorebookJson(proseText);
                    const parsed = entries[0] ?? null;
                    console.debug(`[lorebook] parsed=${parsed ? JSON.stringify(parsed).slice(0, 80) : 'null'}`);
